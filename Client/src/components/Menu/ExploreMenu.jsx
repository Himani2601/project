    ...
    return (
        <>
            <div className='mx-5 mb-10 md:mx-16'>
                ...
                <div className='flex justify-between items-center gap-8 overflow-x-auto flex-wrap'>
                    {menu_list.map((item, index) => {
                        return (
                            <div onClick={() => setCategory(prev => prev === item.menu_name ? "All" : item.menu_name)} key={index} className='text-center hover:cursor-pointer'>
                                <img src={item.menu_image} style={{ width: '120px', height: '120px' }} className={category === item.menu_name ? "border-[5px] rounded-full border-yellow-300" : ""} />
